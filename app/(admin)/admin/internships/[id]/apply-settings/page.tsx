...
import { ArrowLeft, Settings2, AlertCircle, Pencil } from 'lucide-react'
import { getInternshipById } from '@/app/actions/internships'
import { getApplicationSteps } from '@/app/actions/application-steps'
import { ApplicationStepsEditor } from '@/components/admin/application-steps-editor'

type PageProps = {
  params: Promise<{ id: string }>
}

export default async function ApplySettingsPage({ params }: PageProps) {
  const { id } = await params
  
  const [internshipResult, stepsResult] = await Promise.all([
    getInternshipById(id),
    getApplicationSteps(id)
  ])
  ...
  return (
    <div className="p-6 lg:p-8 space-y-5">
      {/* Header - matching edit internship style */}
      <div className="flex items-center gap-4">
        <Link href={`/admin/internships/edit/${id}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Application Form</h1>
            <Badge 
              variant="outline"
