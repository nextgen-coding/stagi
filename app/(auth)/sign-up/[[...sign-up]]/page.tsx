...
import { Loader2, Mail, Lock, User, AlertCircle, Eye, EyeOff, KeyRound } from 'lucide-react'
...
export default function SignUpPage() {
  ...
  const isFromApplication = searchParams.get('from') === 'application'
  ...
  }, [isLoaded, isSignedIn, verifying, redirectUrl, router])

  useEffect(() => {
    // Pre-fill email if coming from application
    if (isFromApplication) {
      const pendingAppId = localStorage.getItem('pendingApplicationId')
      if (pendingAppId) {
        const savedAppData = localStorage.getItem(`application_${pendingAppId}`)
        if (savedAppData) {
          try {
            ...
            if (appData.fullName) {
              const names = appData.fullName.split(' ')
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                required
                maxLength={6}
                className="h-12 pl-10 text-center text-xl tracking-[0.5em] font-mono bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 focus:border-blue-500 focus:ring-blue-500"
              />
  ...
  return (
    <AuthLayout 
      ...
      subtitle={isFromApplication ? "One more step to submit your application!" : "Join thousands of students accelerating their careers."}
    >
      {isFromApplication && (
        <Alert className="mb-6 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20">
          <AlertDescription className="text-blue-800 dark:text-blue-300">
            🎉 <strong>Great! Your application is saved.</strong> Create an account to submit it and track your progress.
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Doe"
              required
              className="h-11 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 focus:border-blue-500 focus:ring-blue-500"
            />
          </div>
        </div>
