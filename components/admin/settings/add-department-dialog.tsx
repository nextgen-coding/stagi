...
import { createDepartment } from '@/app/actions/settings'
...
export function AddDepartmentDialog({ colors, icons }: AddDepartmentDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setIsOpen(false)
        setFormData({ name: '', description: '', color: colors[0].value, icon: icons[0] })
        router.refresh()
      } else {
